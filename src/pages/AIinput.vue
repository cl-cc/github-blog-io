 ...
 * @version: 
 * @Author: 程垒 💻
 * @Date: 2025-02-12 10:05:38
 * @LastEditors: 程
 * @LastEditTime: 2025-09-17 10:10:42
-->
<script setup lang="ts">
import Typed from 'typed.js';
import { onMounted, defineEmits, defineExpose, ref } from 'vue';
...
onMounted(() => {
  ...
  const typed = new Typed('#typedDomInput', options);
});
const contentInput = ref('');

const enterContent = () => {
  emit('sendContent', contentInput.value);
};
const showinput = () => {
  document.getElementById('typedDomInput')!.style.display = 'none';
  //然后让input获取焦点
  document.getElementById('search')!.focus();
};
const blurEvent = () => {
  if (contentInput.value === '') {
    document.getElementById('typedDomInput')!.style.display = 'block';
  }
};
const clearInput = () => {
  contentInput.value = '';
...
<template>
  <div class="ipt">
    <div class="input">
      ...
      <div @click="showinput" id="typedDomInput" v-show="!contentInput" class="tips"></div>
      <input onfocus="if (value==defaultValue)value=''" onblur="if(!value)value=defaultValue" @keyup.enter="enterContent" @blur="blurEvent" id="search" class="search" v-model="contentInput" type="text" />
    </div>
  </div>
</template>
<style lang="scss" scoped>
@import url(./aiInput.scss);
#typedDomInput {
  color: #ccc;
  position: absolute;
  top: 30%;
